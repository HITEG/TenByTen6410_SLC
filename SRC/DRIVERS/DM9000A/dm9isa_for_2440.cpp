...
#ifdef S3_2440
#include <s3c2440a.h>
volatile S3C2440A_INTR_REG *v_pDM9000INTRegs;
volatile S3C2440A_IOPORT_REG *v_pDM9000IOPRegs;
volatile S3C2440A_MEMCTRL_REG *v_pDM9000MEMRegs;
#endif
...
BOOL PROCESSOR_LanInterruptInit(int nINTMode)
{
	...
	U32		val,rdata;
    //--------------
    v_pDM9000IOPRegs = (volatile S3C2440A_IOPORT_REG*)VirtualAlloc(0, sizeof(S3C2440A_IOPORT_REG), MEM_RESERVE, PAGE_NOACCESS);
	if (NULL == v_pDM9000IOPRegs)
	{
		RETAILMSG(TRUE,(TEXT("[DM9ISA]: v_pDM9000IOPRegs is not allocated\n\r")));
	}
	if (!VirtualCopy((PVOID)v_pDM9000IOPRegs, (PVOID)(S3C2440A_BASE_REG_PA_IOPORT >> 8), sizeof(S3C2440A_IOPORT_REG), PAGE_PHYSICAL | PAGE_READWRITE | PAGE_NOCACHE))
	{
		RETAILMSG(TRUE,(TEXT("[DM9ISA]: v_pDM9000IOPRegs is not mapped\n\r")));
	}
	RETAILMSG(TRUE,(TEXT("[DM9ISA]: v_pDM9000IOPRegs is mapped to %x\n\r"), v_pDM9000IOPRegs));

	rdata  = v_pDM9000IOPRegs->GPFUP;  // External Interrupt #7
	rdata |= (1 << 7);
	v_pDM9000IOPRegs->GPFUP = rdata; 
	
	rdata  = v_pDM9000IOPRegs->EXTINT0;
	rdata &= ~(7 << 28); 			// low Level Triggered Mode
	if(INT_ACTIVE_HIGH == nINTMode)
	{
		rdata |= (1 << 28) 			// High Level Triggered Mode
	}
	v_pDM9000IOPRegs->EXTINT0 = rdata;			// High Level Triggered Mode				
#endif
...
// for SMDK6400
#define DM9000x_Tacs	(0x0) //0ns
#define DM9000x_Tcos	(0x1) //8ns
#define DM9000x_Tacc	(0x5) //44ns 
#define DM9000x_Tacp	(0x0) //0ns
#define DM9000x_Tcoh	(0x0) //0ns
#define DM9000x_Tcah	(0x1) //8ns
#define DM9000x_PMC		(0x0) // normal(1data)
...
{    
	...
	U32	val,	rdata;
    //--------------
    v_pDM9000MEMRegs = (S3C2440A_MEMCTRL_REG*)VirtualAlloc(0,sizeof(S3C2440A_MEMCTRL_REG), MEM_RESERVE,PAGE_NOACCESS);
	if(NULL == v_pDM9000MEMRegs) 
	{
		RETAILMSG(TRUE,(TEXT("[DM9ISA]: v_pDM9000MEMRegs is not allocated\n\r")));
	}
	if(!VirtualCopy((PVOID)v_pDM9000MEMRegs,(PVOID)(S3C2440A_BASE_REG_PA_MEMCTRL >> 8),sizeof(S3C2440A_MEMCTRL_REG),PAGE_PHYSICAL | PAGE_READWRITE|PAGE_NOCACHE)) {
		RETAILMSG(TRUE,(TEXT("[DM9ISA]: v_pDM9000MEMRegs is not mapped\n\r")));
	}    
	RETAILMSG(TRUE,(TEXT("[DM9ISA]: v_pDM9000MEMRegs is mapped to %x\n\r"), v_pDM9000MEMRegs));

    // nGCS4=nUB/nLB(nSBHE),nWAIT,16-bit
    rdata = v_pDM9000MEMRegs->BWSCON;
	rdata = rdata & (~(0xf << 16));
	rdata = rdata | (0x1 << 16);	
	v_pDM9000MEMRegs->BWSCON = rdata;

	RETAILMSG (1,(TEXT("----BWSCON=%x\n\r"),v_pDM9000MEMRegs->BWSCON));

#define DM9000_Tacs	(0x3)	// 0clk
#define DM9000_Tcos	(0x3)	// 4clk
#define DM9000_Tacc	(0x7)	// 14clk
#define DM9000_Tcoh	(0x3)	// 1clk
#define DM9000_Tah	(0x3)	// 4clk
#define DM9000_Tacp	(0x3)	// 6clk
#define DM9000_PMC	(0x0)	// normal(1data)

    // BANK4 access timing
	v_pDM9000MEMRegs->BANKCON4 = ((DM9000_Tacs << 13) + (DM9000_Tcos<<11) + (DM9000_Tacc << 8) + (DM9000_Tcoh << 6) + \
		                         (DM9000_Tah << 4) + (DM9000_Tacp << 2) + (DM9000_PMC));
#endif
	return TRUE;
}
...
void PROCESSOR_LanEnableInterrupt(void)
{
    ...
    rdata  = v_pDM9000IOPRegs->GPFCON;
	rdata &= ~(3 << 14);
	rdata |=  (2 << 14);
	v_pDM9000IOPRegs->GPFCON = rdata;			// External Interrupt #7 Enable	
#endif
}
