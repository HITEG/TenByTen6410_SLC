...
class C_Exception
{
    public:
        ...
        {
            MSG((TEXT("DM9 Exception Level(%d)"), _gpExpData->nIndex + 1));
            MSG((m_ndisErrorMessage.Buffer));
        };
        
        U32  GetErrorCode(void)
        {
            return m_uErrorCode;
        };
...
//                NdisMSleep((waited=MINI(uMicroPeriod,remains)));
                NdisStallExecution((waited = MINI(uMicroPeriod, remains)));
            }
...
#define CQueueGetUserPointer(ptr)       ((PVOID)((U32)(ptr) + sizeof(CQUEUE_GEN_HEADER)))
...
class CQueue
{
    public:
        ...
        int Size(void)
        {
            ...
            for (size = 0, pcurr = m_pHead; pcurr; pcurr = pcurr->pNext, size++) {};
